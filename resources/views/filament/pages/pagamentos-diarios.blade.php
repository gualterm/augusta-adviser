...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="/pagamentos-diarios/print?date={{ $this->selectedDate ?: today()->toDateString() }}"
            target="_blank"
            class="inline-flex items-center gap-2 bg-amber-700 hover:bg-amber-800 text-white text-sm font-medium px-4 py-2 rounded-lg transition no-underline">
            🖨️ Imprimir / PDF
        </a>
    </div>
    ...
    @if (!empty($data))
    <div class="print-summary rounded-xl px-6 py-5 border-2 border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
        <div class="flex flex-wrap justify-between items-center gap-4">
            <div>
                <div class="text-xs text-gray-500 uppercase tracking-wide mb-1">Total serviços</div>
                <div class="font-semibold text-gray-700 dark:text-gray-200">€ {{ number_format($grandTotal, 2, ',', '.') }}</div>
            </div>
            <div class="text-right">
                <div class="text-xs text-gray-500 uppercase tracking-wide mb-1">Total a pagar à equipa</div>
                <div class="text-3xl font-bold text-green-700 dark:text-green-400">€ {{ number_format($grandCommission, 2, ',', '.') }}</div>
            </div>
        </div>
    </div>
    @endif
